...
Kategori dağılımı (term/dosya): edu 210/28 · s 102/64 · cc 80/14 · scale 47/15 · sus 42/12 · l1+layer1 37/12 · kernel+k 31/8 · fe 13/9 · stack 12/12 · kalan ~38. *(10–11 Haziran genişlemesindeki 44 aday kayıt bilinçli terimsizdir — `state: aday`; terim yükü ürünleşme kararıyla gelir.)*
...
## 6. Yedi Soru Genellemesi (12 Haziran 2026)

Eğitim sayfalarında page düzeyinde yaşayan yedi soru kalıbı (`enrich.lesson` → "Bu konu yedi soruda") terim düzeyine genellendi: **her glossary kaydı** Ne/Niçin/Nasıl/Nerede/Ne zaman/Kim/Analoji cevaplarının tamamını taşır. Veri, `tools/migrate/seven-questions.json` overlay'inde yaşar (anahtar: glossary term id `term-<slug>-<stem>`; ADR-0005'teki label-anahtarlı overlay'den farkı, kaydın sayfa bağlamına birebir kilitli olmasıdır). Migration bu kayıtları `glossary-detail.json`'a `sevenQuestions` alanı olarak işler; panel (ExplanationPanel) "Yedi soruda" bölümünde gösterir, `analoji` alanı mevcut `realWorldAnalogy` ile uyumlu olduğundan panelde ayrıca analoji bölümü tekrarlanmaz.

Kapsam kapısı `tests/sevenQuestions.test.ts`'tedir ve sayı yazmaz, ilişki zorlar: her glossary terimi için kayıt VAR (eksik=0), yedi alanın tamamı dolu (≥15 karakter), overlay'de sahipsiz anahtar yok. Yeni terim ekleyen her içerik işi bu kapıya takılır — yedi soru cevapları olmadan terim eklenemez. Üretim 12 Haziran 2026'da AI destekli editöryel akışla yapıldı; mevcut 552 `realWorldAnalogy` kaydı birebir korunarak `analoji` alanına taşındı, kalan 157 terim için yeni analoji yazıldı. Done Definition (§4) bu alanlarla genişler: zenginleştirilmiş kayıt artık yedi soru setini de içerir.

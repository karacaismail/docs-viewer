...
## UX Revizyonu — Okunabilirlik ve Sayfa-İçi Araçlar (2026-06: 60+ öğrenen odağı)

Gövde elemanları 72ch satır ölçüsüne sınırlandı (geniş 1280px sütunda okunabilirlik). Sayfa başına eklenenler: ≥4 H2 başlıkta `details` tabanlı "Bu sayfada" içindekiler; üstte 4px okuma ilerleme çubuğu (`ReadingProgress`, dekoratif/aria-hidden); başlıklarda hover/focus'ta görünen çıpa-kopyala düğmesi (pano + aria-live onayı); sağ üstte MD indir + "MD kopyala" ikilisi (kopyala 60+ için "dosya nereye indi?" sorusunu atlar). Görsellerde keşfedilebilir "Büyüt" düğmesi Radix Dialog lightbox açar; wbsChart yatay kaydırma bölgesi kenar solması + açık ipucu cümlesi taşır. `{{ref:}}` bağlantıları hover/focus'ta hedef sayfanın özetini gösteren önizleme kartı sunar (`resolveRefEntry`). Checklist kalıcılık formatı v2'dir: `{ checked, total }` — eski düz dizi okunurken kabul edilir; `CurriculumProgress` `checklist:edu.*` kayıtlarından toplam ilerlemeyi `egitim/edu-overview` ve `egitim/edu-faz-haritasi` sayfalarında özetler.

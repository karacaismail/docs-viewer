...
Yeni terim `glossary.json`'a değil, kaynaktaki page'in `enrich.terms`'üne yazılır — glossary migration'da üretilir, elle düzenlenmez (04 §1 "generated" işareti). Bağlam kuralı: terim hangi page'de açıklanıyorsa o page'in kaydıdır; aynı kelimenin başka bağlamı varsa diğer page kendi kaydını taşır (12 §"Çözümleme"). Zenginleştirme akışı, parti sırası ve bitiş tanımı `12A`'dadır; yazar parti listesini migration raporundan alır. Paragraph içinde terimi etkileşimli yapmak için kelime `term` segment'ine bağlanır — page başına ilk geçiş kuralı (12A §2-B).

## 4. Görsel Eklemek

Görsel dosyası `public/assets/` altına konur, block `image` type'ı ile `src/alt/caption` taşır (07A §3). `alt` zorunludur ve anlam taşır — boş alt, içerik doğrulamada uyarıdır. Kayıp src yayını engellemez ama fallback metin gösterir (07B §1); yazar raporu temiz tutmakla yükümlüdür.

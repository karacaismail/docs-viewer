...
Bu doküman, migration script'inin (`07-uretim-02-data-migration.md`) yazılmasından önce şart olan resmî alan eşlemesini verir. Tablolar tahmine değil taramaya dayanır: kaynak küme programatik olarak analiz edilir (16 Haziran 2026 itibarıyla 197 özgün + 44 aday + 73 karar/kavram kaydıyla toplam 315; ayrıca 1 kapsam dışı dosya). 197 özgün dosya ortak cluster şemasını izler; `ARCHITECTURE-5.json` farklı bir yapıdadır (`$schema`, `meta`, `milestones`…) ve içerik değil spec olduğundan migration kapsamı dışıdır.

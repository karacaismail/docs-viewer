...
import { buildProductionContract, loadUretimOverlays } from "./uretim.mjs";

const ROOT = join(dirname(fileURLToPath(import.meta.url)), "..", "..");
const SRC_DIR = join(ROOT, "content-source");
const OUT_DIR = join(ROOT, "src", "data");
const SKIP = new Set(["ARCHITECTURE-5.json"]); // 07A §5: spec, içerik değil
const URETIM_OVERLAYS = loadUretimOverlays(ROOT); // per-page Üretim Sözleşmesi overlay'leri
...
function transformCluster({ file, stem, data }) {
  ...
  blocks.push(...buildProductionContract(stem, data, nextId, URETIM_OVERLAYS[stem]));
...
function sortItems(results, isEdu = false) {
  const eduOrder = (stem) => {
    if (stem === "edu-baslangic-rotasi") return -3;
    if (stem === "edu-prompt-kutuphanesi") return -2;

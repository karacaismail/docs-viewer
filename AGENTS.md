...
- **SDK** = kernel üstü geliştirme altyapısı (sözleşme + tipli iskelet üretici + CLI + yerel dev-env). Geliştirme yüzeyi: module ve theme tek-komutla scaffold'lanır; app geliştirici tarafından SDK içinde geliştirilir (kompozisyon); ArcheType tek komut + çok-aşamalı AI sorularıyla (seven-questions) üretilir. Runtime'ı kernel sağlar; SDK ve AI kernel iç API'sine dokunamaz. Geliştirici klonlar, tek komutla (sdk up) tüm ortamı kurar.
